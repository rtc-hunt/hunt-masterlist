...
--          let puzzleIdsD = imap (\i _ -> i) <$> puzzleDataDynamic
          ...
          traceShow "Puzzle list started" $ tableDynAttrWithSearch "puzzletable ui celled table"  [
            -- ("Title", \_ _ _ -> puzzleLink (constDyn text "", pure $ (constDyn mempty))
            ("Title", \_ puzDat -> puzzleLink (primaryKey . _puzzleData_puzzle <$> puzDat) $ elAttr "div" ("class" =: "" <> "data-tooltip" =: "Open Puzzle") $ dynText $ _puzzle_Title . _puzzleData_puzzle <$> puzDat, blank ) -- return $ (constDyn (mempty :: PuzzleQuery)))
            , ("Is meta?", \_ puzDat -> dynText $ (\p -> if p then "META" else "") . _puzzle_IsMeta . _puzzleData_puzzle <$> puzDat
              , do
                  dropdownValue <- fmap (flip PuzzleQuery PuzzleOrdering_Any) . _dropdown_value <$> dropdown mempty (constDyn (mempty =: " - " <> PuzzleSelect_IsMeta =: "Is Meta" <> (PuzzleSelect_Not PuzzleSelect_IsMeta) =: "Not Meta")) headerDropdownSettings
                  modifyQuery $ (\(PuzzleQuery old _) (PuzzleQuery new _) -> Endo $ \(PuzzleQuery all ord) -> PuzzleQuery (new <> subPuzzleSelect all old) ord) <$> current dropdownValue <@> updated dropdownValue
                  -- pure dropdownValue
              )
            , ("Meta", \_ puzDat -> void $
...
=======
            ...
            , ("Current Solvers", \_ puzDat _ -> 
              ...
              , do
                  dropdownValue <- fmap (flip PuzzleQuery PuzzleOrdering_Any) . _dropdown_value <$> dropdown mempty (constDyn (mempty =: " - " <> PuzzleSelect_HasSolvers =: "Has Current Solvers" <> (PuzzleSelect_Not PuzzleSelect_HasSolvers) =: "No Current Solvers")) headerDropdownSettings
                  modifyQuery $ (\(PuzzleQuery old _) (PuzzleQuery new _) -> Endo $ \(PuzzleQuery all ord) -> PuzzleQuery (new <> subPuzzleSelect all old) ord) <$> current dropdownValue <@> updated dropdownValue
              ) -}
            , ("Voice Chat", \_ puzDat -> 
                let lnkD = _puzzle_voicelink . _puzzleData_puzzle <$> puzDat
